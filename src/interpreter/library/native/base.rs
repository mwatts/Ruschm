...
fn eqv<R: RealNumberInternalTrait, E: IEnvironment<R>>(
    ...
    match (&a, &b) {
        ...
        (Value::Pair(a), Value::Pair(b)) => Ok(Value::Boolean(match (a.as_ref(), b.as_ref()) {
            ...
            _ => a.as_ref() as *const Pair<R, E> == b.as_ref() as *const Pair<R, E>,
        })),
...
fn library_map_result<R: RealNumberInternalTrait, E: IEnvironment<R>>(
) -> Result<Vec<(String, Value<R, E>)>> {
    Ok(vec![
...
fn builtin_parameters_length() -> Result<()> {
    let sqrt = library_map::<f32, StandardEnv<_>>()
        .into_iter()
        .find(|(name, _)| name.as_str() == "sqrt")
        .unwrap()
        .1
        .expect_procedure()?;
    let newline = library_map::<f32, StandardEnv<_>>()
        .into_iter()
        .find(|(name, _)| name.as_str() == "sqrt")
        .unwrap()
        .1
    ...
    assert_eq!(newline.get_parameters().len(), (1, false));
    Ok(())
}
